...
/* mimic memset */
void SZ_Memset(sizebuf_t * sz, int value, int count)
{
 ...
 * Overload in C++. */
void SZ_Memcpy(sizebuf_t * sz, const Uint8 * buf, int count)
{
	/* State checking */
	if (sz->error || sz->bufferPos + count > sz->bufferLen)
	{
...
void SZ_Memcpy2(sizebuf_t * sz, sizebuf_t * bf, int count)
{
	/* State checking */
	if (sz->error || sz->bufferPos + count > sz->bufferLen)
	{
